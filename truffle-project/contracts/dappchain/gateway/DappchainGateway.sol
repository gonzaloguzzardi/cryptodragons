...
contract DappchainGateway is IERC20Receiver, IERC721Receiver, DappchainValidatorManagerContract {
	...
	mapping(uint256 => bool) lockedDragons;
	...
	event SendDragonToMainchainAttempt(address from, uint256 uid, address toMainchainAddress, bytes data);
	event DragonSuccessfullyRetrievedInSidechain(address sidechainAddress, uint256 uid, bytes data);

	enum TokenKind {ETH, ERC20, ERC721}
	...
	function depositDragon(
		address from,
		address toMainchainAddress,
		uint256 uid,
		bytes memory data
	) public onlyDragonContract {
		// locked dragon to know a transference was made for this id
		lockedDragons[uid] = true;
		balances[from].erc721[from][uid] = data;
		emit SendDragonToMainchainAttempt(from, uid, toMainchainAddress, data);
	}
	...
	) external isVerifiedByValidator(amount, contractAddress, sig) {
	...
	function withdrawERC721(
		...
		bytes calldata sig,
		address contractAddress
	) external isVerifiedByValidator(uid, contractAddress, sig) {
	...
	function withdrawETH(uint256 amount, bytes calldata sig) external isVerifiedByValidator(amount, address(this), sig) {
		balances[msg.sender].eth = balances[msg.sender].eth.sub(amount);
		payable(msg.sender).transfer(amount); // ensure it's not reentrant
	...
	function onERC20Received(address _from, uint256 amount) public override returns (bytes4) {
