...
import "./MainnetECVerify.sol";

contract MainnetValidatorManagerContract is Ownable {
  using MainnetECVerify for bytes32;
  ...
  constructor (address[] memory _validators, uint8 _threshold_num, uint8 _threshold_denom) public {
  ...
  modifier isVerifiedByValidator(uint256 num, address contractAddress, bytes memory sig) {
  ...
  function addValidator(address validator, uint8[] calldata v, bytes32[] calldata r, bytes32[] calldata s)
    external
  {
  ...
  function removeValidator(address validator, uint8[] calldata v, bytes32[] calldata r, bytes32[] calldata s)
    external
  {
